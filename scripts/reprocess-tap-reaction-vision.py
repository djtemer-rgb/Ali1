...
def process_vision(input_path, output_path):
    ...
    print(f"  Sampleed key color: R={kr}, G={kg}, B={kb}")
    ...
    # 2. Slice into 3x2 grid of 418x627 cells
    cell_w = 418
    cell_h = 627
    ...
    for r in range(2):
        for c in range(3):
            # Crop the grid cell from cleaned image
            x0 = c * cell_w
            y0 = r * cell_h
            x1 = x0 + cell_w
            y1 = y0 + cell_h
            ...
            # PIL's getbbox() returns (left, upper, right, lower)
            ...
            if r == 0:
                # Character: align bottom with 15px margin
